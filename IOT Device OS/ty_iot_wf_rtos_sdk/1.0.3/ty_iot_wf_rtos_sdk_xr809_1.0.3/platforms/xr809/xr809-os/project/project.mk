...
ifeq ($(__CONFIG_IMG_COMPRESS), y)
LIBRARIES += -luncompress
endif
...
#         --lzma2=preset=6e,dict=$(XZ_LZMA2_DICT_SIZE),lc=3,lp=1,pb=1
XZ := xz -k --check=crc32 --lzma2=preset=6e,dict=8KiB
XZ_DEFAULT_IMG := $(IMAGE_NAME).img
IMAGE_XZ_CFG ?= $(IMAGE_CFG_PATH)/image$(SUFFIX_WLAN)$(SUFFIX_XIP)$(SUFFIX_IMG_XZ).cfg
endif # __CONFIG_IMG_COMPRESS
...
ifeq ($(__PRJ_CONFIG_IMG_COMPRESS), y)
	...
	xz -k --check=crc32 --lzma2=preset=6e,dict=8KiB $(BIN_PATH)/net.bin
	xz -k --check=crc32 --lzma2=preset=6e,dict=8KiB $(BIN_PATH)/net_ap.bin
...
ifeq ($(__CONFIG_IMG_COMPRESS), y)
	cd $(IMAGE_PATH) && \
	dd if=$(XZ_DEFAULT_IMG) of=$(XZ_DEFAULT_IMG).temp skip=40960 bs=1c && \
