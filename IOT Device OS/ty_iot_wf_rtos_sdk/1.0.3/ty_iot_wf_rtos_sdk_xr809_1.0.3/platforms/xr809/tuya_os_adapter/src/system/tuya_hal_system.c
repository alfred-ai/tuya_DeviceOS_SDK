...
#define portTICK_RATE_MS (OS_MSEC_PER_SEC / OS_HZ)
...
int tuya_hal_get_random_data(uint8_t* dst, int size, uint8_t range)
{
	...
	if(!exec_flag) {
        uint32_t seed_value;
        uint8_t seed_buf[4];
        HAL_Status status = HAL_PRNG_Generate(seed_buf,4);
        if(HAL_OK != status) {
            seed_value = ((seed_buf[0] << 24) | (seed_buf[1] << 16) | (seed_buf[2] << 8) | (seed_buf[3] << 0));
        }
        srand(seed_value);
        exec_flag = TRUE;
    }
